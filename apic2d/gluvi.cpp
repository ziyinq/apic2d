...
namespace Gluvi{
  ...
  Target3D::
  Target3D(scalar target_[3], scalar dist_, scalar heading_, scalar pitch_, scalar fovy_, scalar near_clip_factor_, scalar far_clip_factor_)
  ...
  void Target3D::
  transform_mouse(int x, int y, scalar ray_origin[3], scalar ray_direction[3])
  {
    ...
    scalar camx=(x-0.5*winwidth)*scale, camy=(0.5*winheight-y)*scale, camz=-1.0; // in camera coordinates, this is ray_direction (but not normalized)
  ...
  void Target3D::
  get_viewing_direction(scalar direction[3])
  {
    scalar ch=cos(heading), sh=sin(heading);
    scalar cp=cos(pitch), sp=sin(pitch);
    direction[0]=-sh*cp;
  ...
  TargetOrtho3D::
  TargetOrtho3D(scalar target_[3], scalar dist_, scalar heading_, scalar pitch_, scalar height_factor_, scalar near_clip_factor_, scalar far_clip_factor_)
  ...
  PanZoom2D::
  PanZoom2D(scalar bottom_, scalar left_, scalar height_)
  ...
  void PanZoom2D::
  transform_mouse(int x, int y, scalar coords[2])
  {
    scalar r=height/winheight;
    coords[0]=x*r+left;
