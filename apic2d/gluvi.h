...
struct Target3D : public Camera
{
   ...
   Target3D(scalar target_[3]=0, scalar dist_=1, scalar heading_=0, scalar pitch_=0, scalar fovy_=45,
...
struct TargetOrtho3D : public Camera
{
   ...
   TargetOrtho3D(scalar target_[3]=0, scalar dist_=1, scalar heading_=0, scalar pitch_=0, scalar height_factor_=1,
                 scalar near_clip_factor_=0.01, scalar far_clip_factor_=100);
   void click(int button, int state, int x, int y);
   void drag(int x, int y);
   void return_to_default(void);
